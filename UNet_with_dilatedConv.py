...
# -*- coding: utf-8 -*-
"""
Created on Fri Nov 10 22:55:49 2017

@author: zxue

V1.0 Developing: to repalce the bottom structure "down-conv-up" with "dilated conv rate=2"
...
def unet(input_shape=(240,240,2),bn=True, do=0, ki="he_normal",lr=0.001):
    ...
    conv4 = Conv2D(512, (3,3), dilation_rate=2, padding="same", activation="relu", kernel_initializer=ki)(conv4)
    cat6 = conv4
...
#    model.compile(optimizer=optimizer, loss=dice_coef_loss, metrics=[dice_coef, 'accuracy'])
    ...
    model.compile(optimizer=optimizer, loss=dice_coef_loss, metrics=[dice_coef, precision_xue, recall_xue])
...
def evaluate_testVersion(which_part, input_shape = (240,240,2), weights_path = "weights_240.h5"):
    ...
    eva = model.evaluate(x=X_test, y=Y_test)
...
def predict_testVersion(input_shape = (240,240,2), weights_path = "weights.h5"):
    ...
    for j in range(200):
        ...
        for i in range(Y_test.shape[0]):
            if np.sum(Y_test[i, :,:]) > j * 10:
                slice_index = i
                break
        if slice_index != prev_index:
            ...
            plt.imsave("tmp/"+str(k)+"_pred_precision_"+str(precision)+"_recall_"+str(recall)+".png",y_pred)
            plt.imsave("tmp/"+str(k)+"_gt.png",y_true)
            
            prev_index = slice_index
            k = k + 1
...
#    plt.imshow(y_pred)
#    plt.show()
#    plt.imsave("test_save_pred.png",y_pred)
#    
#    plt.imshow(y_true)
#    plt.show()
#    plt.imsave("test_save_gt.png",y_true)
#
